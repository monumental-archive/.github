...
| Dependencies | **L2** | Met by construction on both archetypes: a commit point refuses every publish job while an advisory in its SBOM is undecided — per release on the versioned path, per publish on the continuous one, which gained the leg in #717 (see `dependency-track.md`; first exercised on `release-lab@v0.20.0` — whose refusal also exposed, and #349 closed, the unwired publish edges) | Exact pins with checksums (`mise.lock`), SHA-pinned actions, 7-day minimum release age, Renovate fan-out, cargo-deny in the gate, signed dependency-keyed VEX as the only exit |
| Build Environment | **L0** | **Formally L0** at both layers: L1's verify-before-instantiation obligation is implemented, its signed-build-image-provenance obligation belongs to producers who do not meet it (see `slsa-reference.md`) | Runner image a named, Renovate-rolled pin, enforced by `lint:runner-pin` (#290 found the signer floating on `ubuntu-latest` while this cell claimed otherwise); pgrx bases digest-pinned, org-attested and verified fail-closed before any container runs; a caller's first-party `FROM` verified against the org signer at the ref its tag implies, before the image build and fail-closed (#715) — the same obligation at the artifact layer, not a level move |
